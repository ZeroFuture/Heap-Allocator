...
inline static size_t mm_units(size_t nbytes) {
    /* smallest count of Node-sized memory chunks */
    /*  (+1 additional chunk for the Node itself) needed to hold nbytes */
    return (nbytes + sizeof(Node) - 1) / sizeof(Node) + 1;
}
...
void *mm_malloc(size_t nbytes) {
    ...
    for (Node *p = prevp->s.ptr; ; prevp = p, p = p->s.ptr) {
...
void mm_free(void *ap) {
    ...
    Node *bp = (Node *)ap - 1;                 /* point to block header */
 ...
 */
static Node *morecore(size_t nu) {
	// nalloc based on page size
	size_t nalloc = mem_pagesize()/sizeof(Node);
    /* get at least NALLOC Node-chunks from the OS */
    if (nu < nalloc) {
        nu = nalloc;
    }
